...
class TfidfOClassSearchEngine:
    def __init__(self, save_path, id2image_path_dict):
        self.object_class = TfidfWrapperEngine(
            save_tfidf_path=save_path,
            id2image_path=id2image_path_dict['object_class'],
            data_type='object_class'
        )
        self.object_number = TfidfWrapperEngine(
            save_tfidf_path=save_path,
            id2image_path=id2image_path_dict['object_number'],
            data_type='object_number'
        )
        self.color_class = TfidfWrapperEngine(
            save_tfidf_path=save_path,
            id2image_path=id2image_path_dict['color_class'],
            data_type='color_class'
        )

    def load_transform(self, transform_type):
        if transform_type == 'object_class':
            context_matrix = self.object_class.context_matrix
            id2image_fps = self.object_class.id2image_fps
            tfidf_transform = self.object_class.tfidf_transform
        elif transform_type == 'object_number':
            context_matrix = self.object_number.context_matrix
            id2image_fps = self.object_number.id2image_fps
            tfidf_transform = self.object_number.tfidf_transform
        elif transform_type == 'color_class':
            context_matrix = self.color_class.context_matrix
            id2image_fps = self.color_class.id2image_fps
            tfidf_transform = self.color_class.tfidf_transform
...
class Bm25OClassSearchEngine:
    def __init__(self, save_path, id2image_path_dict):
        self.object_class = Bm25WrapperEngine(
            save_bm25_path=save_path,
            id2image_path=id2image_path_dict['object_class'],
            data_type='object_class'
        )
        self.object_number = Bm25WrapperEngine(
            save_bm25_path=save_path,
            id2image_path=id2image_path_dict['object_number'],
            data_type='object_number'
        )
        self.color_class = Bm25WrapperEngine(
            save_bm25_path=save_path,
            id2image_path=id2image_path_dict['object_number'],
            data_type='object_number'
        )
    ...
    def load_transform(self, transform_type):
        if transform_type == 'object_class':
            retriever = self.object_class.bm25_retriever
            id2image_fps = self.object_class.id2image_fps
        elif transform_type == 'object_number':
            retriever = self.object_number.bm25_retriever
            id2image_fps = self.object_number.id2image_fps
        elif transform_type == 'color_class':
            retriever = self.color_class.bm25_retriever
            id2image_fps = self.color_class.id2image_fps
...
class OClassSearchEngine:
    ...
    def __call__(self, input_queries, image_path_subset, top_k=100):
        list_results = []
        for input_type in ['object_class', 'color_class', 'object_number']:
