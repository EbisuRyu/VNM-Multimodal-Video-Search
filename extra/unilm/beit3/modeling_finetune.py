...
import extra.unilm.beit3.utils as utils
from extra.unilm.beit3.modeling_utils import BEiT3Wrapper, _get_base_config, _get_large_config
...
class BEiT3ForCaptioning(BEiT3Wrapper):
    ...
    def forward(self, image, text_ids, padding_mask, language_masked_pos, text_len=None, incremental_state=None, **kwargs):
        ...
        uni_mask[t_start:t_end, t_start:t_end] = torch.tril(torch.ones(
            text_len, text_len, dtype=torch.long, device=text_ids.device))
        ...
        if image is None:
            ...
            positions = torch.arange(text_len, text_ids.size(
                1) + text_len, device=text_ids.device).long().unsqueeze(0)
